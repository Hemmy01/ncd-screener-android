...
public class ScreeningRepository {
    ...
    public Screening getScreeningDetailById(int screeningId) {
        ScreeningEntity screeningEntity = screeningDao.getScreeningByIdSync(screeningId);
        if (screeningEntity == null) {
            return null;
        }
        
        // Load related patient and CHW
        PatientEntity patientEntity = patientDao.getPatientByIdSync(screeningEntity.getPatientId());
        CHWEntity chwEntity = chwDao.getCHWById(screeningEntity.getChwId());
        
        Patient patient = patientEntity != null ? EntityConverter.fromEntity(patientEntity) : null;
        CHW chw = chwEntity != null ? EntityConverter.fromEntity(chwEntity) : null;
        
        // Load related observations, conditions, questionnaires, and service requests
