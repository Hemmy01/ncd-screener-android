...
public class LoginActivity extends AppCompatActivity {
    ...
    private void performLogin() {
        ...
        if (username.isEmpty() || password.isEmpty()) {
            ToastHelper.showWarning(this, "Please enter both username and password to continue");
            return;
        }
    ...
    private void authenticateCHW(String username, String password) {
        chwRepository.authenticate(username, password, chw -> {
            if (chw != null) {
                ...
                ToastHelper.showError(this, "Invalid username or password. Please check your credentials and try again");
            }
        });
    }
        ...
        new MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.create_account, (dialog, which) -> {
                ...
                if (username.isEmpty() || first.isEmpty() || last.isEmpty() || pass.isEmpty()) {
                    ToastHelper.showWarning(this, "Please fill in all required fields (username, first name, last name, and password)");
                    return;
                }
                if (!pass.equals(confirm)) {
                    ToastHelper.showError(this, "Passwords do not match. Please ensure both password fields are identical");
                    return;
                }
                ...
                chwRepository.createCHW(newChw, (success, message) -> {
                    if (success) {
                        ToastHelper.showSuccess(this, "Account created successfully! You can now login");
                    } else {
                        ToastHelper.showError(this, message != null ? message : "Failed to create account. Please try again");
                    }
                    if (success) {
        ...
        new MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.reset_password, (dialog, which) -> {
                ...
                if (username.isEmpty() || pass.isEmpty()) {
                    ToastHelper.showWarning(this, "Please fill in username and new password fields");
                    return;
                }
                if (!pass.equals(confirm)) {
                    ToastHelper.showError(this, "Passwords do not match. Please ensure both password fields are identical");
                ...
                chwRepository.resetPassword(username, pass, (success, message) -> {
                    if (success) {
                        ToastHelper.showSuccess(this, "Password reset successfully! You can now login with your new password");
                    } else {
                        ToastHelper.showError(this, message != null ? message : "Failed to reset password. Please check the username and try again");
                    }
                    if (success) {
                        editUsername.setText(username);
                        editPassword.setText(pass);
